...
import { IERC20, IUniswapV2Router02 } from "./interfaces/InterfacesAggregated.sol";
...
contract Treasury is Ownable {
    ...
    function setTaxDistribution(uint _walletCount, address[] calldata _wallets, uint[] calldata _percentDistribution) external onlyOwner {
    ...
    function distributeTaxes() external returns(uint256 _amountToDistribute) {
        ...
        _amountToDistribute = amountRoyaltiesWeth;
    ...
    function viewTaxSettings() external view returns(uint256, address[] memory, uint[] memory) {
        return (
